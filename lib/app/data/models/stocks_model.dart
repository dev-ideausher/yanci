...
// some random stocks
final List<StockModel> randomStockData = List.generate(
  10,
  (index) => StockModel(
    stockName: 'Stock $index',
    fullName: 'Company Name $index',
    price: double.parse((Random().nextDouble() * 1000).toStringAsFixed(2)),
    ...
    points: List.generate(
      ...
      (index) => Point(
        x: index.toDouble(),
        y: double.parse((Random().nextDouble() * 100).toStringAsFixed(2)),
      ),
    ),
    stockFundamentals: StockFundamentals(
      dayHigh: double.parse((Random().nextDouble() * 100).toStringAsFixed(2)),
      shareOut: double.parse((Random().nextDouble() * 1000).toStringAsFixed(2)),
      openPrice: double.parse((Random().nextDouble() * 100).toStringAsFixed(2)),
      week52High: double.parse((Random().nextDouble() * 200).toStringAsFixed(2)),
      dayLow: double.parse((Random().nextDouble() * 100).toStringAsFixed(2)),
